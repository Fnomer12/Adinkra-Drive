 ...
 useEffect(() => {
  async function loadUser() {
    ...
    if (currentUser?.email) {
      ...
      await loadPreviousBookings(currentUser.email);
    } else {
      setPreviousBookings([]);
    }
...
async function loadPreviousBookings(userEmail: string) {
  ...
  try {
    ...
    if (error) {
      console.error("Load previous bookings error:", error);
      return;
    }

    setPreviousBookings(data || []);
  } catch (error) {
    console.error("Previous bookings fetch error:", error);
  } finally {
  ...
  <h3 className="text-lg font-semibold text-gray-900">
    Sign in to view your previous bookings
  </h3>

  {authLoading ? (
    <p className="mt-2 text-sm text-gray-500">Checking your account...</p>
  ) : user ? (
  ...
  <div className="flex h-14 w-14 items-center justify-center rounded-full border border-gray-200 bg-gray-200 text-lg font-semibold text-gray-700">
...
>
  Sign out
    ...
    <button
      ...
      className="mt-3 flex w-full items-center justify-center gap-3 rounded-2xl border border-gray-300 bg-white px-5 py-3 font-semibold text-gray-800 shadow-sm hover:bg-gray-50"
    >
      <svg width="20" height="20" viewBox="0 0 48 48" aria-hidden="true">
        <path
          fill="#EA4335"
          d="M24 9.5c3.54 0 6.72 1.22 9.22 3.6l6.87-6.87C35.6 2.2 30.3 0 24 0 14.7 0 6.4 5.4 2.6 13.3l8 6.2C12.5 13.2 17.8 9.5 24 9.5z"
        />
        <path
          fill="#4285F4"
          d="M46.1 24.5c0-1.6-.1-2.7-.4-3.9H24v7.4h12.6c-.3 2.3-1.9 5.7-5.4 8l8.3 6.5c4.8-4.4 7.6-10.8 7.6-18z"
        />
        <path
          fill="#FBBC05"
          d="M10.6 28.5c-1-3-1-6.2 0-9.2l-8-6.2C.9 16.2 0 20 0 24s.9 7.8 2.6 10.9l8-6.4z"
        />
        <path
          fill="#34A853"
          d="M24 48c6.5 0 12-2.1 16-5.7l-8.3-6.5c-2.2 1.5-5.2 2.5-7.7 2.5-6.2 0-11.5-3.7-13.4-9l-8 6.4C6.4 42.6 14.7 48 24 48z"
        />
      </svg>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <input
                    ...
                    type="email"
                    readOnly={!!user?.email}
                    className="w-full rounded-2xl border border-gray-300 px-4 py-3 outline-none focus:border-black read-only:bg-gray-100"
                    />
              <input
                value={phone}
    ...
    <option value="momo">Mobile Money (MoMo)</option>
        ...
        <aside className="rounded-3xl bg-white p-8 shadow-sm">
          ...
          <div className="mt-6 space-y-4 text-sm text-gray-700">
            ...
            <div className="rounded-2xl bg-gray-50 p-4">
  <p className="text-sm font-semibold text-gray-900">Your Previous Bookings</p>

  {authLoading ? (
    <p className="mt-2 text-sm text-gray-500">Loading account...</p>
  ) : !user ? (
    <p className="mt-2 text-sm text-gray-500">
      Sign in with Google to view your past rentals and purchases.
    </p>
  ) : historyLoading ? (
    <p className="mt-2 text-sm text-gray-500">Loading booking history...</p>
  ) : previousBookings.length === 0 ? (
    <p className="mt-2 text-sm text-gray-500">
      No previous rentals or purchases found.
    </p>
  ) : (
    <div className="mt-3 space-y-3">
      {previousBookings.slice(0, 5).map((booking) => {
        ...
        return (
          ...
          >
            ...
            <p className="text-xs text-gray-500">
              {booking.booking_type === "rent" ? "Rental" : "Purchase"} •{" "}
              {booking.status}
            </p>
