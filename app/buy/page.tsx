...
export default async function BuyPage() {
  ...
  const saleCars = ((data as Vehicle[]) || []).filter(
    (vehicle) => vehicle.category === "sale"
  );
      ...
      <section className="bg-black text-white">
        <div className="mx-auto max-w-7xl px-6 py-20">
          ...
          <p className="mt-4 max-w-2xl text-lg text-gray-300">
            Browse vehicles available for ownership with clear availability and
            trusted details.
          </p>
        </div>
      </section>
        ...
        {saleCars.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-gray-300 bg-white p-10 text-center">
            <h3 className="text-xl font-bold text-gray-900">
              No cars for sale yet
            </h3>
            <p className="mt-3 text-gray-600">
              Upload sale vehicles from the admin dashboard and they will show
              here automatically.
            </p>
          </div>
        ) : (
          <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            {saleCars.map((vehicle) => (
